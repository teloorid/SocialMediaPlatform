...
exports.login = asyncHandler(async (req, res, next) => {
  ...
  try {
    ...
    // Clean expired refresh tokens - with safety check
    if (typeof user.cleanExpiredTokens === 'function') {
    ...
    console.log('User doc keys:', Object.keys(user.toObject()));
...
exports.logout = asyncHandler(async (req, res) => {
  ...
  console.log('Extracted refreshToken variable:', refreshToken);

  // Only try to remove refresh token if one is provided
  if (refreshToken && req.user.refreshTokens) {
    console.log('RefreshToken exists, about to call removeRefreshToken');
    console.log(
      'Before removeRefreshToken - refreshTokens length:',
      req.user.refreshTokens?.length
    );
    ...
    try {
      ...
      console.log(
        'After removeRefreshToken - refreshTokens length:',
        req.user.refreshTokens?.length
      );
      ...
      console.error('Error saving user during logout:', saveError);
      // Continue with logout even if refresh token removal fails
    }
  } else {
    console.log(
      'No refresh token provided or user has no refresh tokens array'
    );
  }
...
exports.logoutAll = asyncHandler(async (req, res) => {
  try {
    ...
    if (req.user.clearAllRefreshTokens) {
      ...
      // Alternative: if you store refresh tokens as an array
...
exports.refreshToken = asyncHandler(async (req, res, next) => {
  ...
  // Find a user with this refresh token
  const user = await User.findOne({
